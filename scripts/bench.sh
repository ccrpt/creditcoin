...
if [[ $BENCH -eq 0 ]]
then
    ./target/release/creditcoin-node benchmark --chain dev --steps=50 --repeat=30 --pallet $PALLET --extrinsic='*' --execution wasm --wasm-execution=compiled --heap-pages=4096 --output ./runtime/src/weights/$PALLET.rs;
fi
